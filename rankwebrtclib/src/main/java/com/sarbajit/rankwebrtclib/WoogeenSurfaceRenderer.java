 ...
 */
public class WoogeenSurfaceRenderer extends SurfaceView
        implements SurfaceHolder.Callback, Stream.VideoRendererInterface {
    ...
    private boolean blackFrame = false;
     ...
     * Standard View constructor. In order to render something, you must first call init().
     */
    public WoogeenSurfaceRenderer(Context context, AttributeSet attrs) {
        super(context, attrs);
        getHolder().addCallback(this);
    }
    ...
    public void init(EglBase.Context sharedContext, RendererCommon.RendererEvents rendererEvents,
        ...
        synchronized (handlerLock) {
            ...
            Log.d(TAG, getResourceName() + "Initializing.");
    ...
    public void release() {
        ...
        synchronized (handlerLock) {
            if (renderThreadHandler == null) {
                Log.d(TAG, getResourceName() + "Already released");
                return;
            }
    ...
    public void surfaceCreated(final SurfaceHolder holder) {
        Log.d(TAG, getResourceName() + "Surface created.");
        synchronized (layoutLock) {
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        Log.d(TAG, getResourceName() + "Surface destroyed.");
        synchronized (layoutLock) {
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        Log.d(TAG, getResourceName() + "Surface changed: " + width + "x" + height);
        synchronized (layoutLock) {
            surfaceSize.x = width;
    ...
    private void renderFrameOnRenderThread() {
        ...
        synchronized (layoutLock) {
            final float[] samplingMatrix = new float[]{1.0F, 0.0F, 0.0F, 0.0F, 0.0F, -1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F, 0.0F, 1.0F};
        ...
        synchronized (statisticsLock) {
            if (framesRendered == 0) {
                ...
                synchronized (layoutLock) {
                    Log.d(TAG, getResourceName() + "Reporting first rendered frame.");
    ...
    private void updateFrameDimensionsAndReportEvents(VideoRenderer.I420Frame frame) {
        synchronized (layoutLock) {
            if (frameWidth != frame.width || frameHeight != frame.height
                ...
                Log.d(TAG, getResourceName() + "Reporting frame resolution changed to "
    ...
    private void logStatistics() {
        synchronized (statisticsLock) {
            Log.d(TAG, getResourceName() + "Frames received: "
                    + framesReceived + ". Dropped: " + framesDropped + ". Rendered: " + framesRendered);
            if (framesReceived > 0 && framesRendered > 0) {
                final long timeSinceFirstFrameNs = System.nanoTime() - firstFrameTimeNs;
                Log.d(TAG, getResourceName() + "Duration: " + (int) (timeSinceFirstFrameNs / 1e6) +
                        " ms. FPS: " + framesRendered * 1e9 / timeSinceFirstFrameNs);
                Log.d(TAG, getResourceName() + "Average render time: "
